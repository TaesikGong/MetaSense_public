...
import sys
sys.path.append('..')
from options import MetaSense_ActivityOpt


device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

feature_flatten_dim = 24576
input_channel_dim = 6
...
class MAML_BN(nn.Module):
    ...
    def __init__(self, opt):
        ...
        conv6 = nn.Conv1d(512, 1024, kernel_size=6, stride=2)
        bn6 = nn.BatchNorm1d(1024)
        conv7 = nn.Conv1d(1024, 2048, kernel_size=6, stride=2)
        bn7 = nn.BatchNorm1d(2048)
        fc1 = nn.Linear(feature_flatten_dim, 2048)
        bn8 = nn.BatchNorm1d(2048)
        fc2 = nn.Linear(2048, 512)
        bn9 = nn.BatchNorm1d(512)
        ...
        layers.append(('conv', conv6))
        layers.append(('bn', bn6))
        layers.append(('conv', conv7))
        layers.append(('bn', bn7))
        layers.append(('fc', fc1))
        layers.append(('bn', bn8))
        layers.append(('fc', fc2))
        layers.append(('bn', bn9))
        layers.append(('fc', fc3))
        layers.append(('bn', bn10))
        layers.append(('fc', fc4))
...
class Extractor(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.feature = nn.Sequential(
            ...
            nn.Conv1d(512, 1024, kernel_size=6, stride=2),
            nn.ReLU(True),
            nn.BatchNorm1d(1024),

            nn.Conv1d(1024, 2048, kernel_size=6, stride=2),
            nn.ReLU(True),
            nn.BatchNorm1d(2048),
        )
...
class Class_Classifier(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.class_classifier = nn.Sequential(
            nn.Linear(feature_flatten_dim, 2048),
            nn.ReLU(True),
            nn.BatchNorm1d(2048),
            ...
            nn.Linear(128, self.opt['num_class']))
