...
class MAML_BN(nn.Module):
    ...
    def forward(self, input, vars=None, training=True):
        ...
        hx = (torch.zeros(1, batch_size, self.opt['num_class']).to(device), torch.zeros(1, batch_size, self.opt['num_class']).to(device))
...
class Extractor(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.lstm = nn.LSTM(128, self.opt['num_class'])
        self.lstm_hidden = self.init_hidden()

    def init_hidden(self, batch_size=1):
        return (torch.zeros(1, batch_size, self.opt['num_class']).to(device),
