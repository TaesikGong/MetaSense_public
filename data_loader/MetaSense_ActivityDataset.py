...
class MetaSense_ActivityDataset(torch.utils.data.Dataset):
    # load static files

    def __init__(self, file='../dataset/metasense_activity/minmax_scaling_all.csv',
                 transform=None, domain=None, activity=None, complementary=False,
                 max_source=100, ecdf=False, num_bin=50):
        """
        Args:
    ...
    def preprocessing(self, ecdf):
        ...
        if self.complementary:
            domains = set(options.MetaSense_ActivityOpt['domains']) - set(self.domain)  # currently supports only user and position
        else:
        ...
        if ecdf:
            num_bin = self.num_bin#opt['ecdf_bin_size']

            ecdf_features = np.zeros((self.features.shape[0], self.features.shape[1], num_bin))
            for row in range(self.features.shape[0]):#for each row
                for axis in range(self.features.shape[1]):#for each sensor axis among 6
                    sorted = np.sort(self.features[row][axis])#sort the features first
                    for i in range(num_bin):
                        num_elem_in_bin = np.count_nonzero((sorted >= i*(1.0/num_bin)) & (sorted < (i+1)*(1.0/num_bin)))
                        ecdf_features[row][axis][i] = num_elem_in_bin/self.features.shape[2]

            self.features = ecdf_features
