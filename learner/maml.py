...
class MAML():
    ...
    def train(self, epoch):
        ...
        num_task = self.num_synthetic_domains

        self.net.train()

        losses_q = [0 for _ in range(self.update_step_train + 1)]  # losses_q[i] is the loss on step i
        confusion_matrices = [np.zeros((self.opt['num_class'], self.opt['num_class']),dtype=int) for _ in range(self.update_step_train + 1)]
        self.reset_layer()



        synthetic_supports, synthetic_queries = self.generate_random_tasks(num_repeat=self.num_synthetic_domains)
        ...
        for task_i in range(num_task): # for each task
            ...
            for k in range(0, self.update_step_train + 1):
                ...
                if k != self.update_step_train: #ignore the last unnecessary update
                    ...
                    class_loss_support, _ = self.get_loss_and_confusion_matrix(self.net, fast_weights, self.class_criterion,
                                                                               input_labeled_spt,
